...
def handle_forecast_postback(messaging_api, event: PostbackEvent) -> bool:
    ...
    if "action=get_weather" in postback_data:
        ...
        try: # 使用 try-except 確保解析安全
            ...
            send_line_reply_message(messaging_api, reply_token, [TextMessage(text="查詢參數解析失敗，請稍後再試。")])
            return True
        ...
        if not all([county, township, days_str]):
            logger.error(f"Postback 數據不完整: {postback_data}")
            send_line_reply_message(messaging_api, reply_token, [TextMessage(text="查詢參數不完整，請稍後再試。")])
            return True
        ...
        weather_data = get_cwa_forecast_data(api_key=CWA_API_KEY, township_name=township) 
        ...
        if not weather_data:
            ...
            send_api_error_message(messaging_api, user_id, reply_token, f"{county}{township}")
            return True

        # 解析天氣數據
        parsed_weather = parse_forecast_weather(weather_data, township)
        ...
        if parsed_weather:
            ...
            logger.info(f"已成功回覆用戶 {user_id} {county}{township} 的 {days} 天天氣預報。")
            clear_user_state(user_id)
            return True
        else:
            logger.error(f"無法從取得的預報資料中解析出 {county}{township} 的天氣資訊，或解析結果不完整。")
            send_api_error_message(messaging_api, user_id, reply_token, f"{county}{township}")
            return True
