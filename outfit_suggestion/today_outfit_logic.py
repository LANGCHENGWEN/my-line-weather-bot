...
def get_outfit_suggestion_for_today_weather(
    ...
    # 這裡假設 hourly_forecast 已經過濾並只包含一個或少數幾個最相關的數據點，
    # 我們取第一個作為「當前」數據。
    current_hour_data = hourly_forecast[0] if hourly_forecast else None
    ...
    if current_hour_data:
        ...
        logger.debug(f"成功提取逐時數據: 體感: {formatted_feels_like} ({apparent_temp_raw}), 濕度: {formatted_humidity} ({humidity_raw}), 風速: {formatted_wind_speed} ({wind_speed_raw}), 風級: {formatted_wind_scale} ({wind_scale_raw})")
    else:
    ...
    min_temp_raw = general_forecast.get("min_temp_raw", None) # 獲取原始數值用於判斷
    ...
    # --- 針對降雨情況進行補充 (更詳細) ---
    if "豪雨" in weather_phenomenon or "大雨" in weather_phenomenon or (precipitation_prob_raw is not None and precipitation_prob_raw > 50): # 假定50%以上為大雨
        ...
        if precipitation_prob_raw is not None and 0 < precipitation_prob_raw <= 50: # 假設 0-50% 為一般陣雨或小雨
