...
從資料庫中讀取所有已設定預設城市並開啟每日推播功能的使用者。
針對每個城市，呼叫天氣資料聚合器來獲取完整的天氣數據，然後使用這些數據生成一個美觀且資訊豐富的 Flex Message。
最後，將這個訊息推播給所有屬於該城市的使用者，確保他們每天都能收到最新的天氣資訊。
"""
import logging
...
def push_daily_weather_notification(line_bot_api_instance):
    ...
    # 1. 從資料庫中批量獲取所有已設定預設城市的使用者
    """
    將使用者按城市分組可以極大的提高效率，而不是為每個使用者單獨查詢一次天氣。
    只需要為每個城市查詢一次天氣數據（`get_today_all_weather_data`），然後將這個天氣數據生成的 Flex Message 重複發送給所有居住在該城市的使用者。
    ...
    for city, user_ids in all_users_by_city.items():
        try:
            ...
            # 4. 為每個使用者發送推播前，再次檢查是否仍啟用推播設定並發送訊息
            """
            雖然 `get_users_by_city` 已經提供了已設定城市的使用者，但使用者隨時可能透過聊天指令關閉推播。
            由於 Firestore 查詢通常有延遲，如果我們依賴一個在推播任務開始時的快照，可能會錯誤的發送訊息給在快照後關閉推播的使用者。
            透過在發送前對每個使用者進行單獨的 `get_user_push_settings` 查詢，可以確保推播決策是基於最新的使用者設定，避免不必要的訊息發送。
            """
            for user_id in user_ids:
                try: # 在發送推播前，會為每個使用者單獨查詢，確認他們是否開啟了 daily_reminder_push
