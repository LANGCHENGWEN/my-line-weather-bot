...
def handle(event):
    """
    這是追蹤事件的處理函式。
    當用戶追蹤或解除封鎖機器人時，這個函式會被觸發。
    負責執行一系列的初始化設定，包括傳送歡迎訊息、設定用戶狀態、以及綁定 Rich Menu。
    """
    ...
    """
    為了引導用戶設定預設城市，這裡將用戶狀態設定為 "awaiting_default_city_input"。
    程式這樣寫是因為這是一種「狀態機」的設計模式。
    當用戶處於這個狀態時，任何文字輸入都會被 `city_input_handler.py` 中的 `handle_awaiting_default_city_input` 函式所處理，直到用戶成功輸入一個有效的縣市名稱為止。
    """
    set_user_state(user_id, "awaiting_default_city_input") # 用戶輸入一個預設城市
    ...
    """
    在用戶追蹤機器人時，自動為他們綁定一個預設的 Rich Menu。
    在用戶還沒有設定預設城市前，他們無法使用某些功能，因此綁定一個 Rich Menu，可以讓他們更容易找到和使用機器人的基本功能。
